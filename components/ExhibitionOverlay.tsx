...
function InfoRow({ icon, label, action }: { icon: React.ReactNode; label: string; action?: React.ReactNode }) {
  return (
...
// ── Slider (rendered only after morph completes) ───────────────────────────────

function ImageSlider({ ex }: { ex: Exhibition }) {
  ...
  return (
    <>
      ...
      >
        <div ref={sliderRef} className="overflow-hidden w-full h-full">
          <motion.div
          ...
          >
            {ex.images.map((src, i) => (
              <div key={i} className="relative shrink-0 h-full" style={{ width: sliderW || '100%' }}>
                <Image src={src} alt={ex.title} fill className="object-cover" priority={i === 0} />
              </div>
            ))}
          </motion.div>
        </div>
          ...
          <>
            ...
            <button
              ...
              className={`absolute right-3 top-1/2 -translate-y-1/2 z-10 size-9 rounded-full bg-white/80 shadow-md flex items-center justify-center transition-opacity duration-150 focus-visible:opacity-100 ${
                hovered && activeIndex < ex.images.length - 1 ? 'opacity-100' : 'opacity-0 pointer-events-none'
              }`}
            >
    ...
    >
      ...
      <div className="shrink-0">
        <motion.div
          ...
          className="relative w-full h-[282px] overflow-hidden bg-tfam-light"
        ...
        {/* Pagination dots rendered by ImageSlider internally — spacer shown during morph */}
        {!morphComplete && ex.images.length > 1 && (
          <div className="h-[18px]" />
        )}
      </div>

      {/* ── Content — not mounted until morph completes; fades in on mount ── */}
      {morphComplete && <motion.div
        initial={{ opacity: 0, y: 8 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.25, ease: 'easeOut' }}
        className="px-[18px] pt-[28px] pb-6 flex flex-col gap-[42px]"
      >
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-1">
              {ex.endDate && (
                <InfoRow icon={<CalendarIcon />} label={`Until ${fmtLong(ex.endDate)}`} />
              )}
              <InfoRow
                icon={<MapPinIcon />}
                label={locationLabel}
              ...
              />
              <InfoRow icon={<ClockIcon />} label={`~${ex.duration} mins to explore`} />
            </div>
          </div>
